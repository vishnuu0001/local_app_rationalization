...
4. Keep `backend/vercel.json` in place (it already routes to `api/index.py`).
   - Important: use either `builds` or `functions` in `vercel.json`, not both (Vercel rejects configs that contain both properties).
5. Add these Environment Variables in **Project Settings → Environment Variables**:
   ...
   - `CORS_ORIGINS=https://<your-frontend-project>.vercel.app` (must be a valid URL; for temporary testing you can use `*`)
...
If deploy succeeds but backend returns `500 FUNCTION_INVOCATION_FAILED`:

- Open Vercel **Functions logs** for the failed request and check the first Python traceback line.
- Recheck env vars for typos, especially `CORS_ORIGINS` (must be URL or `*`) and `DATABASE_PROVIDER`.
- Keep `DATABASE_PROVIDER=sqlite` with `DATABASE_PATH=/tmp/infra_assessment.db` for this SQLite setup.
- Ensure `backend/vercel.json` does not include both `builds` and `functions`.

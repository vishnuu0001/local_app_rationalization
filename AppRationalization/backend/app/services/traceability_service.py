...
class TraceabilityService:
    ...
    def get_traceability_matrix():
        ...
        try:
            ...
            # Pass a sample of the matrix to the LLM (high-redundancy entries first)
            sample = sorted(
                traceability_matrix,
                key=lambda x: {'High': 0, 'Duplicate': 1, 'Unique': 2}.get(x.get('redundancy', 'Unique'), 3)
            )[:20]
            llm_insights = OllamaService.generate_traceability_insights(summary, sample)
            result['llm_insights'] = llm_insights
        except Exception as exc:
            logger.warning("Traceability LLM insights skipped: %s", exc)
